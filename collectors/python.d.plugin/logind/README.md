...
To troubleshoot issues with the `logind` module, run the `python.d.plugin` with the debug option enabled. The output
...
Now you can manually run the `logind` module in debug mode:

```bash
./python.d.plugin logind debug trace
```
