...
To troubleshoot issues with the `ntpd` module, run the `python.d.plugin` with the debug option enabled. The output
...
Now you can manually run the `ntpd` module in debug mode:

```bash
./python.d.plugin ntpd debug trace
```
