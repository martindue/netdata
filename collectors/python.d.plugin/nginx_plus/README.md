...
To troubleshoot issues with the `nginx_plus` module, run the `python.d.plugin` with the debug option enabled. The output
...
Now you can manually run the `nginx_plus` module in debug mode:

```bash
./python.d.plugin nginx_plus debug trace
```
