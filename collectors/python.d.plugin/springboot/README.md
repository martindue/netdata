...
To troubleshoot issues with the `springboot` module, run the `python.d.plugin` with the debug option enabled. The output
...
Now you can manually run the `springboot` module in debug mode:

```bash
./python.d.plugin springboot debug trace
```
