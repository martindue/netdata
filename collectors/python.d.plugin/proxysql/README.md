...
To troubleshoot issues with the `proxysql` module, run the `python.d.plugin` with the debug option enabled. The output
...
Now you can manually run the `proxysql` module in debug mode:

```bash
./python.d.plugin proxysql debug trace
```
